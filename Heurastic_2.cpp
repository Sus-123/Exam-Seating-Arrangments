...
int mcv(vector<vector<vector<pair<string,string>>>>& cls,  int a, int b, int c,  vector<vector<vector<int > > >& visited)
{

    int maxi=INT_MIN ; 
    int cnt=0 ; 

    if(b-1>=0 && c-1>=0 && cls[a][b-1][c-1].first.empty() && visited[a][b-1][c-1] == -1)
    {
       cnt++; 
    }

    if(b+1<m  && c+1<n && cls[a][b+1][c+1].first.empty() && visited[a][b+1][c+1] == -1 )
    {  
       cnt++; 
    }

    if(b-1>=0 && cls[a][b-1][c].first.empty() && visited[a][b-1][c] ==-1 ) 
    {      
       cnt++; 
    }
    if(b+1<m  && cls[a][b+1][c].first.empty() && visited[a][b+1][c] ==-1 ) 
    {            
       cnt++; 
    }

    if(c-1>=0 && cls[a][b][c-1].first.empty() && visited[a][b][c-1] ==-1 )
    {            
        cnt++; 
    }

    if(c+1<n  && cls[a][b][c+1].first.empty() && visited[a][b][c+1] ==-1)
    {             
        cnt++; 
    }

    if(b-1>=0 && c+1<n && cls[a][b-1][c+1].first.empty() && visited[a][b-1][c+1] ==-1 )
    {
        cnt++; 
    }

    if(b+1<m  && c-1>=0 && cls[a][b+1][c-1].first.empty() && visited[a][b+1][c-1] ==-1 )
    {
       cnt++; 
    }
...
{ 
    ...
    for(cordinate p : sv)
    {
        ...
        xx=p.Xx; 
        yy=p.Yy; 
        int curr= mcv(cls, cc, xx, yy, visited);
        if(curr > maxi_mcv)
        {
            v.clear(); 
            maxi_mcv=curr ;
